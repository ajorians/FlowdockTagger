...
bool FlowAPILibrary::Tag( FlowdockAPI pFlow, const std::string& strOrg, const std::string& strFlow, const std::string& strUsername, const std::string& strPassword, int nThreadId, const std::string& strTag )
{
   FlowdockTagFunc Tag = (FlowdockTagFunc)m_lib.Resolve( "FlowdockTag" );
   if ( !Tag )
      return false;

   Tag( pFlow, strOrg.c_str(), strFlow.c_str(), strUsername.c_str(), strPassword.c_str(), nThreadId, strTag.c_str() );
   return true;
}
...
std::string FlowAPILibrary::Listen(FlowdockAPI pFlow, std::string& strUser, int& nThreadID)
{
   ...
   if( nType == 0 || nType == 1/*Comment*/ /*|| nType == 3*//*Tag-change*/ )
   {
